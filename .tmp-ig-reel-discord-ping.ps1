...
# Does not print or require DISCORD_WEBHOOK_URL. Safe to leave running until target time.
$ErrorActionPreference = "Stop"
$logPath = Join-Path $PSScriptRoot ".tmp-ig-reel-discord-ping.log"
$lockPath = Join-Path $PSScriptRoot ".tmp-ig-reel-discord-ping.lock"
$contactUrl = "https://japaneselanguagementor.com/api/contact"
$hubUrl = "https://japaneselanguagementor.com/api/feature-report"

# --- Edit per arm ---
# Aug 4, 2026 18:30 Pacific Daylight Time = 2026-08-05 01:30 UTC
$targetUtc = [DateTime]::SpecifyKind([DateTime]::Parse("2026-08-05T01:30:00"), [DateTimeKind]::Utc)
$clipTitles = "BatchA2 / ichiban-suki (一番/好き mistake->fix hook)"
$ytPinComment = @"
...
Post Shorts/Reels now (6:30 PM Los Angeles / Pacific).
Clip(s): $clipTitles
...
if (Test-Path $lockPath) {
  ...
  Write-Log "Another reminder job may already be running (lock: $existing). Exiting."
  exit 0
}

Set-Content -Path $lockPath -Value ("pid={0}; started={1}" -f $PID, (Get-Date -Format o))
Write-Log "Scheduled Shorts/Reels Discord + Teacher Hub ping. Target UTC: $($targetUtc.ToString('o'))"
